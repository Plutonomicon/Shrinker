...
import Hedgehog                    (Gen)
import Hedgehog.Gen                (integral,choice,list,text,ascii,bytes)
import Hedgehog.Range              (linear)
import PlutusCore.Data             (Data(I,B,List,Map,Constr)) 
import PlutusCore.DeBruijn         (DeBruijn (..), Index (..))
import PlutusCore.Default          (DefaultFun (..), DefaultUni, Some(Some), ValueOf(ValueOf))
import UntypedPlutusCore.Core.Type (Term(Constant,Builtin,Error,Var,LamAbs,Force,Delay,Apply))
