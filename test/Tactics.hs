...
import Shrink(size,defaultShrinkParams)
import Shrink.Types 
import Shrink.Names

import Test.Tasty (TestTree,testGroup)
import Test.Tasty.Hedgehog (testProperty)
import Hedgehog   (MonadTest, annotate, failure, success,forAll,property,assert)

import PlutusCore.Evaluation.Machine.ExMemory   (ExCPU (..), ExMemory (..))
import PlutusCore.Evaluation.Machine.ExBudget   (ExBudget (..), ExRestrictingBudget (..))
import UntypedPlutusCore.Evaluation.Machine.Cek (RestrictingSt(RestrictingSt),CekEvaluationException,restricting,runCekNoEmit)
...
shrinkingTactics = testGroup "shrinking tactics" (
